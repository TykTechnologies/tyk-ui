  ...
  return (
    <>
      ...
      <Modal className="custom-modal-class" opened={showModal} onClose={() => setShowModal(false)} theme={theme}>
        ...
        <Modal.Body>
          {!showDeprecated && (
            <h5>Save your changes before leaving?</h5>
          )}
          <p>You are about to leave this page, any changes that haven't been saved will be lost.</p>
        </Modal.Body>
        <Modal.Footer>
          <Button theme="secondary-outline" onClick={() => setShowModal(false)}>Close</Button>
          <Button theme="secondary" onClick={() => setShowModal(false)}>Save Changes</Button>
        </Modal.Footer>
      </Modal>
      ...
      <Modal className="custom-modal-class" opened={showBigModal} onClose={() => setShowModal(false)} theme={theme}>
        ...
        <Modal.Footer>
          <Button theme="secondary-outline" onClick={() => setShowBigModal(false)}>Close</Button>
          <Button theme="secondary" onClick={() => setShowBigModal(false)}>Save Changes</Button>
        </Modal.Footer>
      </Modal>
    </>
  );
